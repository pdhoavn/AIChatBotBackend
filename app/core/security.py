...
def is_admin(user: Users) -> bool:
    ...
    permission_names = {
        p.permission_name.lower() for p in user.permissions if p.permission_name
    }
...
def verify_content_manager_leader(user: Users) -> bool:
    ...
    return user.content_manager_profile and user.content_manager_profile.is_leader
...
def verify_consultant_leader(user: Users) -> bool:
    ...
    # Check if is_leader flag is set
    return user.consultant_profile and user.consultant_profile.is_leader
...
async def get_current_user(request: Request, db: Session = Depends(get_db)) -> Any:
    ...
    try:
        ...
        # ---------------------------------------------------------
        # LUỒNG 1: NẾU LÀ NGƯỜI DÙNG CHAT (Bỏ qua Database hoàn toàn)
        # ---------------------------------------------------------
        if token_data.role == "chat_user":
            # Chỉ cần trả về 1 dictionary chứa email
            # Các API gọi Chat chỉ cần biết email là đủ
            return {
        ...
        user = (
            ...
            .filter(Users.email == token_data.email)
            .first()
        )
        ...
        if user:
            print(
                f"DEBUG: Loaded {len(user.permissions)} permissions for user {user.user_id}"
            )
