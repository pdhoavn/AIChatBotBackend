...
class LiveChatService:
    ...
    def register_customer_sse(self, customer_id: int, callback):
        ...
        print(
            f"Registering SSE for customer {customer_id}. Current connections: {current_count}"
        )

        self.sse_customers.setdefault(customer_id, []).append(callback)
        print(
            f"Customer {customer_id} now has {len(self.sse_customers[customer_id])} SSE connection(s)"
        )

    def register_official_sse(self, official_id: int, callback):
        current_count = len(self.sse_officials.get(official_id, []))
        print(
            f"Registering SSE for official {official_id}. Current connections: {current_count}"
        )

        self.sse_officials.setdefault(official_id, []).append(callback)
        print(
            f"Official {official_id} now has {len(self.sse_officials[official_id])} SSE connection(s)"
        )
    ...
    def unregister_customer_sse(self, customer_id: int, callback):
        if customer_id in self.sse_customers:
            try:
                ...
                print(
                    f"Unregistered SSE for customer {customer_id}. Remaining: {remaining}"
                )
    ...
    def unregister_official_sse(self, official_id: int, callback):
        if official_id in self.sse_officials:
            try:
                ...
                print(
                    f"Unregistered SSE for official {official_id}. Remaining: {remaining}"
                )
    ...
    def get_sse_connection_count(
        self, customer_id: int = None, official_id: int = None
    ):
    ...
    async def customer_join_queue(self, customer_id: int):
        ...
        try:
            ...
            queue_entry = LiveChatQueue(
                customer_id=customer_id, status="waiting", created_at=datetime.now()
            )
            db.add(queue_entry)
            db.commit()
            ...
            await self.send_customer_event(
                customer_id,
                {
                    "event": "queued",
            ...
            for oid in list(self.sse_officials.keys()):
                await self.send_official_event(oid, {"event": "queue_updated"})
    ...
    async def official_accept(self, official_id: int, queue_id: int):
        ...
        try:
            ...
            session = ChatSession(session_type="live", start_time=datetime.now())
            db.add(session)
            db.commit()
            db.refresh(session)
            ...
            # Create participants
            participant1 = ParticipateChatSession(
                user_id=customer_id, session_id=session_id
            )
            participant2 = ParticipateChatSession(
                user_id=official_id, session_id=session_id
            )
            ...
            print(
                f"[Accept] Sending 'accepted' SSE to customer {customer_id} with session_id={session_id}"
            )
            await self.send_customer_event(
                customer_id,
                {
            ...
            await self.send_official_event(official_id, {"event": "queue_updated"})
    ...
    async def official_reject(self, official_id: int, queue_id: int, reason: str):
        ...
        # notify student
        await self.send_customer_event(
            queue_item.customer_id, {"event": "rejected", "reason": reason}
        )
    ...
    async def join_chat(self, websocket, session_id: int):
        ...
        print(
            f"[Join Chat] Session {session_id} now has {connection_count} active connection(s)"
        )
    ...
    async def broadcast_message(self, session_id: int, sender_id: int, message: str):
        ...
        try:
            print(
                f"[Broadcast] Saving message to DB: session_id={session_id}, sender_id={sender_id}, message='{message}'"
            )
            ...
            print(
                f"[Broadcast] Message saved with interaction_id={chat.interaction_id}"
            )
            ...
            print(
                f"[Broadcast] Active connections for session {session_id}: {len(active_connections)}"
            )

            if len(active_connections) == 0:
                print(
                    f"[Broadcast] WARNING: No active WebSocket connections for session {session_id}!"
                )
    ...
    async def leave_chat(self, websocket, session_id: int):
        ...
        if session_id in self.active_sessions:
            if websocket in self.active_sessions[session_id]:
                ...
                print(
                    f"[Leave Chat] Connection removed. Remaining connections: {remaining}"
                )
            else:
                print(
                    f"[Leave Chat] WARNING: WebSocket not found in session {session_id}"
                )
            ...
            if not self.active_sessions[session_id]:
                ...
                print(
                    f"[Leave Chat] Session {session_id} has no more connections, removed from active_sessions"
                )
        else:
            print(
                f"[Leave Chat] WARNING: Session {session_id} not found in active_sessions"
            )
    ...
    async def end_session(self, session_id: int, ended_by: int):
        ...
        try:
            session = (
                db.query(ChatSession).filter_by(chat_session_id=session_id).first()
            )
            if not session:
            ...
            for p in all_participants:
                profile = (
                    db.query(AdmissionOfficialProfile)
                    .filter_by(admission_official_id=p.user_id)
                    .first()
                )
                if profile:
    ...
    def get_queue_list(self, official_id: int):
        ...
        items = (
            db.query(LiveChatQueue)
            .filter(LiveChatQueue.status == "waiting")
            .options(joinedload(LiveChatQueue.customer))
            .all()
        )
        official = (
            db.query(AdmissionOfficialProfile)
                ...
                "customer": (
                    {
                        "full_name": (
                            ...
                            if item.customer
                            else f"Customer {item.customer_id}"
                        ),
                        "email": item.customer.email if item.customer else "N/A",
                        "phone_number": (
                            item.customer.phone_number if item.customer else "N/A"
                        ),
                    }
                    if item.customer
                    else {
        ...
        try:
            ...
            for session in active_sessions_query:
                ...
                customer_participant = (
                    db.query(ParticipateChatSession, Users.full_name)
                    .join(Users, ParticipateChatSession.user_id == Users.user_id)
                    .filter(
                        ParticipateChatSession.session_id == session_id,
                        ParticipateChatSession.user_id != official_id,
                    )
                    ...
                    result.append(
                        {
                            ...
                            "start_time": (
                                start_time.isoformat() + "T00:00:00"
                                if start_time
