...
class TrainingService:
    ...
    @staticmethod
    def get_university_full_name(unit: Optional[str] = None) -> str:
        """
        Trả về tên đầy đủ, chính thức của trường/phân hiệu dựa theo đơn vị (UTC | UTC2).
        Dùng trong các prompt xưng danh Nhà trường (VD: "Bạn là chatbot của trường {...}").
        Mặc định fallback về UTC2 nếu unit rỗng/không khớp — giữ đúng hành vi cũ trước khi có unit.
        """
        mapping = {
            "UTC2": "Phân Hiệu Trường Đại học Giao thông Vận tải tại Thành Phố Hồ Chí Minh (UTC2)",
            "UTC": "Trường Đại học Giao thông Vận tải (UTC)",  # TODO: bạn xác nhận/chỉnh lại tên chính thức
        }
        return mapping.get(unit, mapping["UTC2"])

    @staticmethod
    def get_university_query_keyword(unit: Optional[str] = None) -> str:
        """
        Trả về cụm từ khóa cơ sở dùng để chèn vào câu truy vấn đã enrich,
        khớp với "QUY TẮC BẢO TOÀN TỪ KHÓA CƠ SỞ" / Nhóm Cơ sở trong enrich_query.
        Mặc định fallback về UTC2 nếu unit rỗng/không khớp.
        """
        mapping = {
    ...
    @staticmethod
    def get_department_reference_block_context(unit: Optional[str] = None) -> str:
        """
        Khối "Thông tin các phòng ban tham khảo" — bản RÚT GỌN,
        dùng trong stream_response_from_context (chỉ Đào tạo + Khảo thí).
        """
        blocks = {
            "UTC2": """Phòng Đào tạo
    Tổ chức và quản lý đào tạo các hệ.
    Xếp thời khóa biểu, xếp lịch thi, rà soát kế hoạch học tập năm học.
    Giải quyết các vấn đề về học tập; cấp các loại giấy xác nhận (xác nhận sinh viên, xác nhận hoàn thành chương trình học, xác nhận tốt nghiệp tạm thời...); văn bằng, chứng chỉ; điểm thi; mở lớp; thi cử; phục vụ các thủ tục hành chính.
    ...
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-dao-tao
    ...
    Địa chỉ: Phòng 108, 109, 110 Nhà E10, số 451 Lê Văn Việt, P. Tăng Nhơn Phú, TP. Hồ Chí Minh
    Điện thoại: 028 3896 2819; 028 3730 6883; 028 3730 7120
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-khao-thi-va-dam-bao-chat-luong-dao-tao
    Email: bankt-dbcl@utc2.edu.vn""",
            "UTC": """PHÒNG ĐÀO TẠO ĐẠI HỌC
            Địa chỉ: 
        ...
        return blocks.get(unit, blocks["UTC2"])

    @staticmethod
    def get_department_reference_block_full(unit: Optional[str] = None) -> str:
        """
        Khối "Thông tin các phòng ban tham khảo" — bản ĐẦY ĐỦ,
        dùng trong stream_response_from_NA (toàn bộ các phòng ban).
        """
        blocks = {
            "UTC2": """Phòng Tổ chức Hành chính
    Quản lý công tác tổ chức, nhân sự, hành chính và văn thư.
    ...
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-to-chuc-hanh-chinh
    Email: tochuchanhchinh@utc2.edu.vn

    Đào tạo
    Tổ chức và quản lý đào tạo các hệ.
    Khảo sát kỳ tăng cường.
    Xếp thời khóa biểu, xếp lịch thi, rà soát kế hoạch học tập năm học.
    Quản lý kết quả học tập, xét học bổng, cảnh báo học vụ, tốt nghiệp.
    Giải quyết các vấn đề về học tập, điểm thi, mở lớp, thi cử, văn bằng, chứng chỉ, xác nhận sinh viên (để vay vốn, làm thủ tục hành chính.
    Cấp bảng điểm, xử lý các quyết định liên quan đến sinh viên như chuyển điểm, thôi học, chuyển hệ.
    ...
    Giải quyết các vấn đề về quản lý sinh viên, ngoại khóa, chế độ chính sách, khen thưởng, kỷ luật, và xác nhận tạm hoãn nghĩa vụ quân sự
    Quản lý thẻ sinh viên, cố vấn học tập, đánh giá rèn luyện.
    Xét và đề nghị kỷ luật sinh viên khi vi phạm.
    Địa chỉ: Phòng 6, Phòng 15 Nhà D3, số 450 Lê Văn Việt, P. Tăng Nhơn Phú, TP. Hồ Chí Minh
    Điện thoại: 028 3736 0564
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-cong-tac-chinh-tri-va-sinh-vien
    ...
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-khoa-hoc-cong-nghe-and-doi-ngoai
    Email: stic@utc2.edu.vn

    Phòng Thiết bị - Quản trị
    Tham mưu về quy hoạch, đầu tư, mua sắm, sửa chữa, quản lý cơ sở vật chất, tài sản công và PCCC.
    ...
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-thiet-bi-quan-tri
    Trong dữ liệu bạn gửi chưa thấy ghi rõ số điện thoại của phòng này.

    Phòng Tài chính - Kế toán
    Giải quyết các vấn đề về học phí, lệ phí.
    ...
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/phong-tai-chinh-ke-toan
    ...
    Địa chỉ: Nhà E1, khu Giảng đường, số 451 Lê Văn Việt, P. Tăng Nhơn Phú, TP. Hồ Chí Minh
    Điện thoại: 028 3730 9469
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/ban-thanh-tra
    ...
    Quản lý tài liệu, giáo trình, phục vụ nhu cầu học tập và nghiên cứu của sinh viên, giảng viên.
    ...
    Website: https://utc2.edu.vn/chi-tiet-bai-viet/ban-quan-ly-ky-tuc-xa
    Fanpage: https://www.facebook.com/ktxutc2""",
            "UTC": "Hệ thống hiện chưa cập nhật dữ liệu phòng ban tham khảo của cơ sở UTC (Hà Nội).",  
        }
        return blocks.get(unit, blocks["UTC2"])
...
"""
    ...
    async def enrich_query(self, session_id: str, user_message: str, unit: Optional[str] = None) -> str:
        ...
        university_fullname = self.get_university_full_name(unit)
        university_query_keyword = self.get_university_query_keyword(unit)
        ...
        2. KHÔNG BỊA ĐẶT: TUYỆT ĐỐI KHÔNG thêm thông tin hoàn toàn mới chưa từng được nhắc đến. KHÔNG thay đổi mục tiêu câu hỏi. Quy tắc này áp dụng cả khi chèn từ đồng nghĩa ở mục 9: chỉ được chèn từ, KHÔNG được suy diễn thêm giai đoạn/hoàn cảnh/mục đích khác câu hỏi gốc (ví dụ: câu hỏi đang ở giai đoạn "xin trước khi việc xảy ra" thì không được tự suy diễn thành "việc đã xảy ra rồi xin xử lý sau").
        ...
        CƠ SỞ HIỆN TẠI TRONG PHIÊN CHAT: {unit}
            - {university_query_keyword}
                → Khi gặp bất kỳ từ nào, thêm vào query: {university_fullname}

                Ví dụ (giả sử cơ sở đang chọn là UTC2):
                - User hỏi: "quy định thi tại Phân hiệu"
                → Query: "quy định công tác thi kết thúc học phần tại Phân hiệu TP.HCM(Thành Phố Hồ Chí Minh)"
                ...
                - User hỏi câu có chữ "trường" trong câu: "địa chỉ của trường"
            ...
            - NẾU câu hỏi gốc có nhắc tên một cơ sở KHÁC với cơ sở đã chọn (VD: đang chọn UTC nhưng câu hỏi nhắc "UTC2" hoặc ngược lại), BẮT BUỘC vẫn giữ nguyên tên cơ sở NGƯỜI DÙNG vừa nhắc trong câu hỏi gốc (không tự ý sửa lại theo cơ sở đã chọn), vì đây có thể là câu hỏi so sánh hoặc hỏi chéo giữa 2 cơ sở.
            - NẾU câu hỏi gốc KHÔNG nhắc đến từ khóa cơ sở nào: TUYỆT ĐỐI KHÔNG tự ý suy diễn hay thêm các từ "Phân hiệu", "UTC2", "mã GSA" vào câu query.
        ...
        7. QUY TẮC XỬ LÝ TRUY VẤN TÌM KIẾM NHÂN SỰ (ĐỌC KỸ ĐỂ PHÂN BIỆT):
        + TRƯỜNG HỢP 1 - TÌM THEO TÊN NGƯỜI: 
        NẾU câu hỏi CÓ CHỨA TÊN RIÊNG CỦA MỘT CON NGƯỜI THẬT (Ví dụ: "Đặng Văn Ơn là ai?", "ThS Nguyễn Văn A làm gì?"):
        -> BẮT BUỘC bọc TÊN RIÊNG đó trong dấu ngoặc kép "".
        -> BẮT BUỘC bổ sung thêm các cụm từ: "chức vụ", "phòng ban", "đơn vị công tác".
        -> Ví dụ: User hỏi "ông đặng văn ơn có chức vụ gì" => Viết lại: Tìm chức vụ, đơn vị công tác và phòng ban của "Đặng Văn Ơn".

        + TRƯỜNG HỢP 2 - TÌM NGƯỜI GIỮ CHỨC VỤ (ĐIỀU KIỆN CẤM THÊM TỪ KHÓA NHIỄU): 
        NẾU người dùng hỏi xem AI ĐANG GIỮ MỘT CHỨC VỤ nào đó (kể cả chức vụ chung chung như "giám đốc" hay chức vụ gắn liền với đơn vị như "trưởng khoa công trình") HOẶC KHÔNG có tên người thật:
        -> TUYỆT ĐỐI KHÔNG bọc ngoặc kép chức danh.
        -> VIẾT LẠI câu truy vấn theo cấu trúc sau để hệ thống dễ tra cứu: "Tìm thông tin nhân sự, chức danh lãnh đạo của đơn vị: [Tên đơn vị/Chức vụ]".
        ...
        10. QUY TẮC GIẢI MÃ TỪ VIẾT TẮT VÀ TIẾNG LÓNG HỌC ĐƯỜNG:
            - Người học và sinh viên có thói quen dùng từ viết tắt hoặc ngôn ngữ mạng trong câu hỏi gốc. Bạn BẮT BUỘC phải sử dụng kiến thức nền của mình để dịch và viết đầy đủ nghĩa của các từ này ra dạng văn bản hành chính chính thức, giúp máy tìm kiếm không bị nhận diện sai lệch vector.
            - Hướng dẫn ánh xạ một số từ viết tắt phổ biến (nhưng không giới hạn):
              + "nvqs" hoặc "nvquas" ──> "nghĩa vụ quân sự"
              + "sv" ──> "sinh viên"
              + "gv" ──> "giảng viên"
              + "ctctsv" hoặc "ctct&sv" ──> "công tác chính trị và sinh viên"
              + "đgrl" hoặc "dgrl" ──> "đánh giá rèn luyện"
              + "hp" ──> Tùy ngữ cảnh cuộc hội thoại để dịch thành "học phần" hoặc "học phí"
              + "tkb" ──> "thời khóa biểu"
        ...
        - Nhóm Tạm dừng (chỉ khớp khi đi kèm từ "học"/"học tập" trong câu gốc): "nghỉ học", "bảo lưu", "nghỉ học tạm thời", "tạm dừng học", "hoãn học"
    ...
    async def enrich_query_tuyensinh(self, session_id: str, user_message: str, unit: Optional[str] = None) -> str:
        ...
        prompt = f"""
        Bạn là một trợ lý chuẩn hóa truy vấn cho chatbot RAG tư vấn tuyển sinh của {university_name}.
        ...
        6. Viết lại câu hỏi mới nhất thành một câu truy vấn ĐỘC LẬP, ĐẦY ĐỦ NGỮ NGHĨA để máy tìm kiếm (Vector Search) có thể hiểu được chính xác mà không cần đọc lại lịch sử.
            + TRƯỜNG HỢP 1 (Dữ liệu đặc thù): NẾU câu hỏi liên quan đến CÁC THÔNG SỐ CỤ THỂ CỦA NGÀNH HỌC (Ví dụ: Điểm chuẩn, Điểm trúng tuyển, Chỉ tiêu, Tên ngành, Mã ngành, Tổ hợp môn). -> BẮT BUỘC thêm cụm "{key_word}" vào cuối câu hỏi.
            + TRƯỜNG HỢP 2 (Dữ liệu dùng chung): NẾU câu hỏi liên quan đến QUY TRÌNH & THỦ TỤC (Ví dụ: Hồ sơ, Giấy tờ, Các bước nộp, Lệ phí, Tiêu chuẩn cộng điểm, Học phí, Ký túc xá, Quy chế). -> BẮT BUỘC GIỮ NGUYÊN câu hỏi gốc (TUYỆT ĐỐI KHÔNG thêm {key_word}).
    ...
    async def llm_relevance_check(
        self, enriched_query: str, matched_question: str, answer: str, message: str, unit: Optional[str] = None
    ) -> bool:
        university_name = self.get_university_full_name(unit)
        print(f"câu trả lời: {answer}")
        prompt = f"""
        Bạn là chuyên gia đánh giá giữa câu hỏi tìm kiếm, câu hỏi trong cơ sở dữ liệu và câu trả lời, đánh giá độ phù hợp cho 1 hệ thống chat RAG tuyển sinh của trường {university_name}.

        Câu hỏi người dùng gốc(Mục đích để hiểu giọng điệu và yêu cầu trực tiếp):
        {message}
        Câu hỏi người dùng đã được enrich(Mục đích để đối chiếu Câu hỏi DB trong đa dạng tình huống):
        {enriched_query}
        ...
        Trước khi sử dụng bất kỳ đoạn Context nào để trả lời, BẮT BUỘC phải kiểm tra xem đoạn Context đó thuộc về cơ sở nào.

        Nếu UNIT là UTC (Cơ sở chính): TUYỆT ĐỐI KHÔNG SỬ DỤNG các thông tin/đoạn văn bản có chứa các từ khóa như "Phân hiệu", "UTC2", "TP. Hồ Chí Minh", "PH.HCM".

        Nếu UNIT là UTC2 (Phân hiệu): BẮT BUỘC phải dùng tài liệu của Phân hiệu.
        Nếu Context chỉ có thông tin của cơ sở khác với UNIT hiện tại, BẮT BUỘC trả về "nope"
        ...
        - LỆCH CƠ SỞ (QUAN TRỌNG): Context vi phạm quy tắc Unit (chỉ áp dụng riêng cho cơ sở khác).
        (LƯU Ý: TUYỆT ĐỐI KHÔNG đánh giá "false" chỉ vì câu hỏi của người dùng là từ khóa ngắn/rộng mà câu trả lời DB lại chi tiết. Nếu thông tin chi tiết đó đúng mục tiêu tìm kiếm, vẫn phải là "true").
        3. Đặc biệt:
    ...
    async def backfill_target_units_to_utc2(self):
        """
        Gán target_units = ["UTC2"] cho toàn bộ điểm trong Qdrant
        hiện đang thiếu field target_units (tài liệu/QA được tạo trước khi
        có tính năng phân biệt cơ sở UTC/UTC2).
        """
        empty_units_filter = models.Filter(
            must=[
                models.IsEmptyCondition(
                    is_empty=models.PayloadField(key="target_units")
                )
            ]
        )

        # --- Collection tài liệu ---
        doc_result = await self.async_qdrant_client.set_payload(
            collection_name="knowledge_base_documents",
            payload={"target_units": ["UTC2"]},
            points=empty_units_filter,
        )
        print(f"[Qdrant] Updated documents collection: {doc_result}")


    async def llm_listing_check(self, query: str, unit: Optional[str] = None) -> bool:
        university_name = self.get_university_full_name(unit)
        prompt = f"""
        Bạn là chuyên gia phân loại câu hỏi cho hệ thống chatbot tuyển sinh của trường {university_name}.
        ...
        Trả về "true" nếu câu hỏi yêu cầu liệt kê liên quan đến toàn bộ, tất cả, Trọn bộ, Đầy đủ, Tổng,... ví dụ:
        - (Tổng, toàn bộ, tất cả, ...)Danh sách ngành đào tạo ("trường có những ngành gì", "liệt kê các ngành", "có bao nhiêu ngành")
        - (Tổng, toàn bộ, tất cả, ...)Chỉ tiêu từng ngành ("chỉ tiêu các ngành năm nay", "mỗi ngành lấy bao nhiêu người")
        - (Tổng, toàn bộ, tất cả, ...)Điểm chuẩn tất cả các ngành ("điểm chuẩn các ngành", "năm ngoái ngành nào lấy bao nhiêu điểm")
        - (Tổng, toàn bộ, tất cả, ...)Học phí từng ngành / toàn bộ chương trình ("học phí các ngành", "bảng học phí")
        - (Tổng, toàn bộ, tất cả, ...)Các phương thức xét tuyển ("có những phương thức xét tuyển nào")
        - (Tổng, toàn bộ, tất cả, ...)Danh sách tổ hợp môn ("các tổ hợp môn xét tuyển", "tổ hợp nào được dùng")
        - (Tổng, toàn bộ, tất cả, ...)Danh sách học bổng / chính sách hỗ trợ ("có những loại học bổng nào")
        - (Tổng, toàn bộ, tất cả, ...)Danh sách cơ sở / campus ("trường có mấy cơ sở", "các cơ sở đào tạo")
        - (Tổng, toàn bộ, tất cả, ...)Bất kỳ câu hỏi nào dùng từ: "tất cả", "toàn bộ", "các ... là gì", "liệt kê", "danh sách", "có những ... nào", "bao nhiêu ngành/chuyên ngành"
    ...
    async def llm_admission_check(self, query: str, unit: Optional[str] = None) -> bool:
        university_name = self.get_university_full_name(unit)
        prompt = f"""
        Bạn là chuyên gia phân loại câu hỏi cho hệ thống chatbot tuyển sinh của trường {university_name}.
        ...
        Nhiệm vụ:
        Xác định câu hỏi này có hỏi về thông tin TUYỂN SINH (dành cho người muốn thi/xét tuyển đầu vào) hay không.
        ...
        Trả về "false" nếu câu hỏi KHÔNG liên quan đến tuyển sinh (thường là Quy chế học vụ cho người ĐÃ LÀ SINH VIÊN hoặc công việc nội bộ):
        - CHUYỂN NGÀNH, CHUYỂN HỆ (VD: kỹ sư sang cử nhân, chính quy sang vhvl), CHUYỂN TRƯỜNG, HỌC SONG NGÀNH (văn bằng 2).
    ...
    async def llm_document_recommendation_check(
        self, enriched_query: str, context: str, unit: Optional[str] = None,
    ) -> bool:
        ...
        === QUY TẮC NGẦM HIỂU HỆ ĐÀO TẠO ===
            ...
            - Tên file đôi khi sẽ có những tên không dấu, hay viết tắt, hãy chú ý ngữ cảnh và nội dung
        === LUẬT PHÂN LOẠI ===
        ...
        - Trả về "nope" CHỈ KHI Context rơi vào các trường hợp sau:
            ...
            3. SAI HỆ ĐÀO TẠO (QUAN TRỌNG): Hãy chú ý đến thẻ [Nguồn: ...] và nội dung theo sau. Nếu tài liệu (hoặc tên nguồn) ghi rõ là dành cho các hệ khác (như Vừa làm vừa học, Từ xa, Thạc sĩ, Sau đại học...) mà KHÔNG có thông tin cho hệ người dùng yêu cầu (hoặc hệ Chính quy mặc định), BẮT BUỘC trả về "nope". 
            4. LỆCH CƠ SỞ (QUAN TRỌNG): Context vi phạm quy tắc Unit (chỉ áp dụng riêng cho cơ sở khác).
        === DỮ LIỆU ĐẦU VÀO ===
    ...
    async def stream_response_from_context(
        ...
        try:
            ...
            memory = memory_service.get_memory(session_id)
            mem_vars = memory.load_memory_variables({})
            department_reference_block_context = self.get_department_reference_block_context(unit)
            university_name = self.get_university_full_name(unit)
            chat_history = mem_vars.get("chat_history", "")
            full_response = ""
            suggestion = None
            ...
            QUY TẮC XƯNG HÔ VÀ SỞ HỮU DỮ LIỆU (PERSONA & DATA OWNERSHIP):
                Nhập vai tuyệt đối: Bạn là Trợ lý ảo của Nhà trường, toàn bộ thông tin trong Context là "Não bộ" và "Cơ sở dữ liệu của hệ thống". Người dùng cuối KHÔNG cung cấp tài liệu cho bạn.
                CÁC TỪ NGỮ BỊ CẤM (TUYỆT ĐỐI KHÔNG DÙNG): "theo dữ liệu bạn cung cấp", "trong đoạn ngữ cảnh", "theo văn bản bạn gửi", "tài liệu trên".
                CÁC TỪ NGỮ BẮT BUỘC DÙNG ĐỂ THAY THẾ: "Theo thông tin hiện có trên hệ thống...", "Theo quy định của Nhà trường...", "Theo dữ liệu của...".
            === PHONG CÁCH TRẢ LỜI ===
            Cách trả lời:
            ...
            Cách phản hồi:
                ...
                - ĐIỀU KIỆN GỢI Ý PHÒNG BAN(Luôn kiểm tra ĐIỀU KIỆN GỢI Ý PHÒNG BAN vì gợi ý khá quan trọng khi mà 1 số câu trả lời chưa chắc sẽ đúng): 
                  + Bước 1: Phân tích chủ đề câu hỏi và đối chiếu với danh sách "Thông tin các phòng ban tham khảo" ở bên dưới.
                  + Bước 2 (NẾU CÓ): Nếu có phòng ban liên quan trực tiếp, BẮT BUỘC gợi ý để người dùng liên hệ
                  + Bước 3 (NẾU KHÔNG): Nếu trong danh sách KHÔNG CÓ phòng ban nào thực sự phù hợp với câu hỏi và nội dung trả lời, TUYỆT ĐỐI KHÔNG gợi ý gượng ép, KHÔNG được tự bịa tên hay thông tin của các phòng ban ngoài danh sách. Bỏ qua phần gợi ý phòng ban.
            === Thông tin các phòng ban bạn có thể tham khảo để gợi ý nếu cần thiết tùy tình hướng ===
                {department_reference_block_context}
            ...
            5. - LOGIC TƯ DUY TỪNG BƯỚC CHO CÂU HỎI XÁC NHẬN (CÓ/KHÔNG):
                + Khi người dùng nêu một tình huống/hành vi (ngay cả khi không có dấu "?" hay từ "có được không"), BẮT BUỘC phải ngầm hiểu đó là câu hỏi yêu cầu xác nhận.
                + TRƯỚC KHI TRẢ LỜI, BẮT BUỘC THỰC HIỆN 3 BƯỚC SUY LUẬN SAU:
                  - BƯỚC 1 (Tìm điều kiện): Đọc tài liệu để xem hành vi đó yêu cầu điều kiện gì (VD: Xét học bổng thì cần điểm Khá trở lên).
                  - BƯỚC 2 (Đối chiếu thực tế): Xét xem đối tượng người dùng hỏi CÓ ĐÁP ỨNG ĐƯỢC điều kiện đó không. 
                    [ĐẶC QUYỀN KIẾN THỨC]: Bạn ĐƯỢC PHÉP dùng kiến thức thực tế sau để suy luận mà không vi phạm quy tắc chống ảo giác: "Tân sinh viên học kỳ 1 (Ví dụ K66 mới nhập học) CHẮC CHẮN CHƯA THỂ CÓ Điểm trung bình học tập và Điểm rèn luyện".
                  - BƯỚC 3 (Trả lời tự nhiên kèm giải thích):
                    -> LỆNH CẤM: TUYỆT ĐỐI KHÔNG mở đầu cộc lốc bằng các từ đơn như: "Có", "Không", "Được", "Không được".
                    -> BẮT BUỘC phải lồng ghép trực tiếp KẾT QUẢ (được phép hay không) vào ngay câu đầu tiên một cách tự nhiên, sau đó mới giải thích lý do. và gợi ý phòng ban liên quan để biết thêm thông tin
                    -> Nếu thực tế KHÔNG THỂ đáp ứng hoặc BỊ CẤM: Trả lời theo hướng "Bạn chưa đủ điều kiện / không được phép [nhắc lại hành động] vì [giải thích lý do]..." và gợi ý phòng ban liên quan để biết thêm thông tin
                    -> Nếu thỏa mãn: Trả lời theo hướng "Bạn có thể / được phép [nhắc lại hành động] vì [giải thích lý do]..." và gợi ý phòng ban liên quan để biết thêm thông tin chi tiết
                    -> Nếu tài liệu không đủ cơ sở kết luận: Trả lời "Hệ thống hiện chưa có thông tin xác nhận cụ thể cho trường hợp này." và gợi ý phòng ban liên quan để biết thêm thông tin
                + LỆNH CẤM: TUYỆT ĐỐI KHÔNG dùng văn phong lấp liếm kiểu "Có, nếu đáp ứng đủ điều kiện..." đối với những trường hợp hiển nhiên không thể đáp ứng trên thực tế (như sinh viên kỳ 1 xét học bổng khuyến khích).
            ...
            12. QUY TẮC ĐÓNG KHUNG KIẾN THỨC (ZERO-HALLUCINATION & STRICT RAG):
                Bạn là một cỗ máy đọc hiểu. BẮT BUỘC chỉ được phép trả lời dựa trên ĐÚNG NHỮNG GÌ CÓ TRONG ĐOẠN "THÔNG TIN THAM KHẢO" (Context) được cung cấp.
                LỆNH CẤM: TUYỆT ĐỐI KHÔNG SỬ DỤNG kiến thức có sẵn (pre-trained knowledge) của bạn để suy diễn, bịa đặt, hoặc hướng dẫn những quy trình, thủ tục không hề có trong Context (Ví dụ: Thủ tục cơ quan nhà nước, Luật pháp chung, Quy trình ngoài trường...).
            13. QUY TẮC PHÂN BIỆT HỆ ĐÀO TẠO khi CÂU HỎI NGƯỜI DÙNG có chủ đề liên quan đến bằng tiếng anh như TOEIC, IELTS,... 
                - Nếu câu hỏi của người dùng KHÔNG chỉ định rõ hệ đào tạo (như Thạc sĩ, Cao học, Đào tạo từ xa), bạn BẮT BUỘC phải ngầm hiểu người dùng đang hỏi về hệ "Đại học chính quy(dai hoc chinh quy, dao tao trinh do dai hoc)". TRƯỚC KHI TRẢ LỜI, BẮT BUỘC phải kiểm tra tên "Nguồn" của THÔNG TIN THAM KHẢO. TUYỆT ĐỐI KHÔNG được lấy dữ liệu từ nguồn của các thông tin tham khảo có các chữ trong tên nguồn như: (có dấu, không dấu, viết tắt) ["thạc sĩ", "thac si", "cao học", "cao hoc", "từ xa", "tu xa", "vừa làm", "vua lam", "vlvh", "dttx", "tt23"] để trả lời cho hệ Đại học chính quy. BẠN BẮT BUỘC PHẢI COI TOÀN BỘ ĐOẠN ĐÓ LÀ DỮ LIỆU BỊ CẤM. Và theo đó BẮT BUỘC trả lời rằng hệ thống chưa có dữ liệu cụ thể,
                và đề nghị liên hệ Phòng Đào tạo. TUYỆT ĐỐI KHÔNG được lấy số liệu từ nguồn đã bị
                loại, kể cả khi đó là nguồn duy nhất có số liệu.
            === HƯỚNG DẪN XỬ LÝ LƯU Ý ===
            - Dựa vào thông tin tham khảo trên được cung cấp
            - Chỉ sử dụng "đoạn hội thoại trước" để hiểu ngữ cảnh câu hỏi, không dùng "đoạn hội thoại trước" làm nguồn thông tin trả lời. Nếu trong "đoạn hội thoại trước" có câu trả lời hợp lệ với câu hỏi hiện tại nhưng lại khác "Cơ sở" thì TUYỆT ĐỐI CẤM LẤY ĐÓ LÀM CÂU TRẢ LỜI vì là khác cơ sở
    ...
    KHÔNG tự suy đoán hoặc copy năm từ câu hỏi của người dùng nếu context không xác nhận.
            ...
            Câu hỏi người dùng gốc(Mục đích để hiểu giọng điệu và yêu cầu trực tiếp):
            {message}
            Câu hỏi người dùng đã được enrich(Mục đích để đối chiếu chính xác với Context):
            {query}
            """
            full_response = ""
...
"""
    ...
    async def stream_response_from_context_tuyensinh(
        ...
        try:
            ...
            Cách trả lời:
                ...
                - VAI TRÒ CHUẨN MỰC: Bạn đang đại diện cho Nhà trường (Trường Đại học) để trả lời sinh viên/phụ huynh/cán bộ/giảng viên. Văn phong cần chuyên nghiệp, lịch sự, đúng mực nhưng vẫn gần gũi.
                - Trả lời bằng tiếng Việt hoặc tiếng Anh dựa vào câu hỏi người dùng là tiếng Việt hay tiếng Anh
                - Dùng ngôn ngữ đời thường
                - Dùng Markdown linh hoạt: chỉ dùng tiêu đề ## và gạch đầu dòng khi câu trả lời có nhiều mục rõ ràng. Câu trả lời ngắn thì viết thành đoạn văn tự nhiên, không cần chia heading.
    ...
    async def stream_response_from_qa(
        ...
        try:
            ...
            Cách phản hồi:
                ...
                - KIỂM TRA ĐIỀU KIỆN GỢI Ý PHÒNG BAN: 
                  + Bước 1: Phân tích chủ đề câu hỏi và đối chiếu với danh sách "Thông tin các phòng ban tham khảo" ở bên dưới.
                  + Bước 2 (NẾU CÓ): Nếu có phòng ban liên quan trực tiếp, BẮT BUỘC gợi ý để người dùng liên hệ
                  + Bước 3 (NẾU KHÔNG): Nếu trong danh sách KHÔNG CÓ phòng ban nào thực sự phù hợp với câu hỏi và nội dung trả lời, TUYỆT ĐỐI KHÔNG gợi ý gượng ép, KHÔNG được tự bịa tên hay thông tin của các phòng ban ngoài danh sách. Bỏ qua phần gợi ý phòng ban.
            === Thông tin các phòng ban bạn có thể tham khảo để gợi ý nếu cần thiết tùy tình hướng ===
    ...
    async def stream_response_from_NA(
        ...
        try:
            ...
            mem_vars = memory.load_memory_variables({})
            university_name = self.get_university_full_name(unit)
            department_reference_block_full_content = self.get_department_reference_block_full(unit)
            chat_history = mem_vars.get("chat_history", "")
            audience = (
            ...
            prompt = f"""
            Bạn là tầng phản hồi của chatbot tra cứu thông tin {filtered_audience_names} của trường {university_name} khi mà danh mục {filtered_audience_names} hiện tại không có thông tin mà người dùng cần.
            CHÚ Ý ĐẶC BIỆT NGHIÊM TRỌNG: UTC VÀ UTC2 LÀ 2 CƠ SỞ KHÁC NHAU, UTC LÀ TRƯỜNG ĐẠI HỌC GIAO THÔNG VẬN TẢI VÀ UTC2 LÀ PHÂN HIỆU TRƯỜNG ĐẠI HỌC GIAO THÔNG VẬN TẢI TẠI TPHCM
            CƠ SỞ CỦA PHIÊN CHAT HIỆN TẠI: {unit}
            ...
            Cách phản hồi:
                - Gợi ý thông tin liên quan hữu ích
                - Nếu có đường dẫn liên quan đến nội dung người dùng muốn biết, có thể gợi ý để họ tự tìm hiểu thêm, TUYỆT ĐỐI không được lấy đường dẫn không liên quan đến nội dung trả lời
                - ĐIỀU KIỆN GỢI Ý PHÒNG BAN: 
                  + Bước 1: Phân tích chủ đề câu hỏi và đối chiếu với danh sách "Thông tin các phòng ban tham khảo" ở bên dưới.
                  + Bước 2 (NẾU CÓ): Nếu có phòng ban liên quan trực tiếp, BẮT BUỘC gợi ý đầy đủ thông tin phòng ban đó để người dùng liên hệ
                  + Bước 3 (NẾU KHÔNG): Nếu trong danh sách KHÔNG CÓ phòng ban nào thực sự phù hợp với câu hỏi, TUYỆT ĐỐI KHÔNG gợi ý gượng ép, KHÔNG được tự bịa tên hay thông tin của các phòng ban ngoài danh sách. Bỏ qua phần gợi ý phòng ban.
            === Thông tin các phòng ban bạn có thể tham khảo để gợi ý nếu cần thiết tùy tình hướng ===
                {department_reference_block_full_content}
            === KỈ LUẬT THÉP ===
            1. BẢO VỆ TÍNH CHÍNH XÁC CỦA ĐƠN VỊ VÀ LIÊN HỆ (CẤM RÂU ÔNG NỌ CẮM CẰM BÀ KIA):
                - Khi bạn khuyên người dùng liên hệ một đơn vị/phòng ban cụ thể (Ví dụ: Phòng Đào tạo, Phòng Khảo thí), BẮT BUỘC chỉ được cung cấp thông tin liên hệ (SĐT, Email) CỦA CHÍNH XÁC PHÒNG BAN ĐÓ (phải khớp tên).
                - LỆNH CẤM THAY THẾ: TUYỆT ĐỐI KHÔNG copy số điện thoại của các phòng ban khác (như Tuyển sinh, Tổ chức hành chính, CTCTSV...) dán vào để "chữa cháy" hoặc "gợi ý thêm" khi không tìm thấy số của phòng ban cần thiết.
                - CÁCH XỬ LÝ KHI THIẾU DATA: Nếu khuyên liên hệ "Phòng X" nhưng trong context KHÔNG CÓ số điện thoại của "Phòng X", BẮT BUỘC chỉ dừng lại ở lời khuyên và nói rõ: "Hiện tại hệ thống chưa có thông tin liên hệ trực tiếp của phòng này, bạn vui lòng tra cứu trên website trường nhé." (CẤM tự động liệt kê số của Tổ chức hành chính/Đường dây nóng ra để bù đắp).
            2. CÁCH XỬ LÝ KHI KHÔNG TÌM THẤY THÔNG TIN TRONG CONTEXT:
                - TUYỆT ĐỐI KHÔNG sử dụng kiến thức bên ngoài để "gợi ý thêm", "đoán", "hướng dẫn chung chung", hay đề xuất "mình có thể giúp bạn soạn/viết...". Việc tỏ ra hữu ích bằng kiến thức ngoài hệ thống là VI PHẠM NGHIÊM TRỌNG.
            === HƯỚNG DẪN TRẢ LỜI ===
    ...
    async def search_documents(
        ...
        must_conditions = [
            models.FieldCondition(
                key="audience_ids",
                match=models.MatchValue(value=audience_ids),
            )
        ]
        if intent_id:
            must_conditions.append(
                models.FieldCondition(
                    key="intent_id",
                    match=models.MatchValue(value=intent_id),
                )
            )
        if unit:
            must_conditions.append(
    ...
    def filter_public_content(self, chunks: list) -> list:
        return [
            chunk
            for chunk in chunks
            if hasattr(chunk, "payload")
            and not chunk.payload.get("is_private", False)
        ]
