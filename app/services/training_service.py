...
class TrainingService:
    ...
    async def enrich_query(self, session_id: str, user_message: str) -> str:
        ...
        9. - NHÓM CƠ SỞ HCM: "Phân hiệu", "UTC2", "Cơ sở II", "Cơ sở 2", "CSII", "CS2", "TP.HCM", "Phân hiệu TP.HCM", "cơ sở tại thành phố"
            → Khi gặp bất kỳ từ nào, thêm vào query: "Phân hiệu TP.HCM(UTC2)"
        ...
        try:
            ...
            5. - LOGIC TƯ DUY TỪNG BƯỚC CHO CÂU HỎI XÁC NHẬN (CÓ/KHÔNG):
                ...
                + TRƯỚC KHI TRẢ LỜI, BẮT BUỘC THỰC HIỆN 3 BƯỚC SUY LUẬN SAU:
                  ...
                  - BƯỚC 3 (Kết luận dứt khoát):
                    -> Nếu thực tế KHÔNG THỂ đáp ứng (như kỳ 1 chưa có điểm): BẮT BUỘC trả lời mở đầu bằng "Dạ KHÔNG ĐƯỢC." và giải thích lý do thực tế. 
                    -> Nếu nội dung quy định cấm: BẮT BUỘC mở đầu bằng "Không được phép."
                    -> Nếu thỏa mãn: Trả lời "Dạ ĐƯỢC."
                    -> Nếu tài liệu hoàn toàn không có cơ sở để kết luận: Trả lời "Hệ thống hiện chưa có thông tin xác nhận cụ thể cho trường hợp này."
