    ...
    async def stream_response_from_context(
        ...
        try:
            ...
            5. - LOGIC ĐỒNG NHẤT CHO CÂU HỎI CÓ/KHÔNG (YES/NO QUESTIONS):
                ...
                + TRƯỜNG HỢP CONTEXT CHỈ CÓ QUY ĐỊNH CHUNG, KHÔNG CÓ CÂU NÀO NHẮC TRỰC TIẾP ĐẾN ĐÚNG ĐỐI TƯỢNG/HỌC KỲ/KHÓA NGƯỜI DÙNG HỎI:
                LƯU Ý QUAN TRỌNG VỀ ĐIỀU KIỆN KÍCH HOẠT: "Đã có xác nhận trực tiếp" CHỈ tính khi CÙNG MỘT câu/đoạn văn vừa nêu tên đối tượng cụ thể (VD: "K66", "học kỳ 1") VỪA nêu rõ nội dung trả lời cho câu hỏi (VD: được/không được xét học bổng). Việc tên đối tượng xuất hiện ở MỘT CHUNK KHÁC, nói về CHỦ ĐỀ KHÁC (VD: chunk chỉ liệt kê năm nhập học, thủ tục hoãn nghĩa vụ quân sự...) KHÔNG được tính là "đã có xác nhận", dù trùng tên đối tượng. TUYỆT ĐỐI KHÔNG được ghép hai thông tin từ hai chunk không liên quan nhau lại để tạo thành một kết luận xác nhận.
                    -> TUYỆT ĐỐI KHÔNG tự suy luận ra kết luận Có/Không/Vẫn có thể/Cũng được xét... thay người dùng, dù quy định chung nghe có vẻ áp dụng được cho trường hợp đó.
                    -> BẮT BUỘC mở đầu bằng: "Hệ thống hiện chưa có thông tin xác nhận cụ thể cho trường hợp này."
                    -> CẤM các cụm suy diễn: "vẫn có thể được xét nếu...", "cũng được xét nếu đáp ứng đủ điều kiện...", "về nguyên tắc là được...", "nên có khả năng..."
                - KỸ NĂNG XỬ LÝ NGOẠI LỆ(chỉ áp dụng "KỸ NĂNG XỬ LÝ NGOẠI LỆ" khi Context có nêu RÕ RÀNG trường hợp ngoại lệ cho đúng đối tượng đang hỏi): Khi người dùng hỏi về một "Đối tượng/Tình huống cụ thể", BẮT BUỘC kiểm tra xem nó chịu sự chi phối của "Quy định chung" hay nằm trong "Trường hợp ngoại lệ" của tài liệu.
