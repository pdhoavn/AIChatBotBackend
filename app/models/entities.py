...
class Users(Base):
    ...
    permissions = relationship(
        ...
        overlaps="user,user_permissions",
    )
    ...
    consultant_profile = relationship(
        "ConsultantProfile", back_populates="user", uselist=False
    )
    content_manager_profile = relationship(
        "ContentManagerProfile", back_populates="user", uselist=False
    )
    ...
    articles = relationship(
        "Article", back_populates="author_user", cascade="all, delete-orphan"
    )
    admission_informations = relationship(
        "AdmissionInformation", back_populates="creator", cascade="all, delete-orphan"
    )
    admission_forms = relationship(
        "AdmissionForm", back_populates="user", cascade="all, delete-orphan"
    )

    # chat, recommendations
    chat_interactions = relationship(
        "ChatInteraction", back_populates="user", cascade="all, delete-orphan"
    )
...
class UserPermission(Base):
    ...
    permission_id = Column(
        Integer, ForeignKey("Permission.permission_id"), primary_key=True
    )
...
class CustomerProfile(Base):
    ...
    academic_scores = relationship(
        "AcademicScore", back_populates="customer", cascade="all, delete-orphan"
    )
    riasec_results = relationship(
        "RiasecResult", back_populates="customer", cascade="all, delete-orphan"
    )
...
class Interest(Base):
    ...
    customer_profiles = relationship(
        "CustomerProfile", back_populates="interest", cascade="all, delete-orphan"
    )
...
class Major(Base):
    ...
    admission_forms = relationship(
        "AdmissionForm", back_populates="major", cascade="all, delete-orphan"
    )
    articles = relationship(
        "Article", back_populates="major", cascade="all, delete-orphan"
    )
    specializations = relationship(
        "Specialization", back_populates="major", cascade="all, delete-orphan"
    )
...
class ChatSession(Base):
    ...
    interactions = relationship(
        "ChatInteraction", back_populates="session", passive_deletes=True
    )
    participate_sessions = relationship(
        "ParticipateChatSession", back_populates="session", cascade="all, delete-orphan"
    )
...
class ParticipateChatSession(Base):
    ...
    user_id = Column(Integer, ForeignKey("Users.user_id"), primary_key=True)
    session_id = Column(
        Integer, ForeignKey("ChatSession.chat_session_id"), primary_key=True
    )
...
class ChatInteraction(Base):
    ...
    session_id = Column(
        Integer,
        ForeignKey("ChatSession.chat_session_id", ondelete="SET NULL"),
        nullable=True,
    )
...
class Intent(Base):
    ...
    faq_statistics = relationship(
        "FaqStatistics", back_populates="intent", cascade="all, delete-orphan"
    )
    training_questions = relationship(
        "TrainingQuestionAnswer", back_populates="intent", cascade="all, delete-orphan"
    )
    document = relationship(
        "KnowledgeBaseDocument", back_populates="intent", cascade="all, delete-orphan"
    )
...
class TargetAudience(Base):
    ...
    intents = relationship(
        "Intent", back_populates="target_audience", cascade="all, delete-orphan"
    )
...
class TrainingQuestionAnswer(Base):
    ...
    created_by_user = relationship(
        ...
        back_populates="training_question_answers_created",
    )
...
class KnowledgeBaseDocument(Base):
    ...
    author = relationship(
        "Users", foreign_keys=[created_by], back_populates="knowledge_documents"
    )
...
class Template(Base):
    ...
    qa_pairs = relationship(
        "Template_QA", back_populates="template", cascade="all, delete-orphan"
    )
...
class Specialization(Base):
    ...
    articles = relationship(
        "Article", back_populates="specialization", cascade="all, delete-orphan"
    )
...
class Article(Base):
    ...
    status = Column(
        String, default="draft"
    )  # Values: draft, published, rejected, cancelled
    create_at = Column(Date, default=datetime.now)
    created_by = Column(Integer, ForeignKey("Users.user_id"))
    major_id = Column(Integer, ForeignKey("Major.major_id"), nullable=True)
    specialization_id = Column(
        Integer, ForeignKey("Specialization.specialization_id"), nullable=True
    )
